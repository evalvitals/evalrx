...
def lora_available() -> bool:
    """Whether the optional ``peft`` dependency (``pip install evalvitals[finetune]``)
...
def run_lora_repair(
    ...
    *in place* (via ``peft.get_peft_model``) and unloaded again in a
    ...
    if not lora_available():
        return LoraRepairResult(error="peft is not installed (pip install evalvitals[finetune])")
