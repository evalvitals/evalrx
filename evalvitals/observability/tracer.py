"""DiagnosticTracer — Native Langfuse & OpenTelemetry Tracing Engine for EvalVitals.
...
class DiagnosticTracer:
        ...
        if self._langfuse_client is not None:
            try:
                self._live_root = self._langfuse_client.start_observation(
                    ...
                    name=f"EvalVitals: {model} on {benchmark}",
                    as_type="chain",
                    input=self.trace_metadata,
                    metadata=self.trace_metadata,
                )
                self._apply_tags(["evalvitals", model, benchmark])
            except Exception as exc:
    ...
    def export_bundle(self, out_path: str | Path | None = None) -> dict[str, Any]:
        ...
        bundle = {
            "trace": {
                "id": self.trace_id,
                "name": f"EvalVitals: {self.trace_metadata.get('model', 'Model')}",
                "metadata": self.trace_metadata,
    ...
    for candidate in (run_dir / "run_log.jsonl", run_dir / "logs" / "run_log.jsonl"):
        if candidate.exists():
            try:
                ...
                pass
            break
    return hashlib.md5((fingerprint or "evalvitals").encode("utf-8")).hexdigest()


def export_to_langfuse_bundle(run_dir: str | Path, out_json: str | Path | None = None) -> dict[str, Any]:
    """Convert an EvalVitals run artifacts directory into a rich Langfuse bundle."""
    ...
    trace = {
        ...
        "tags": ["evalvitals", run["model"], run["benchmark_name"], run["stopped_by"]],
        "metadata": {
            "model": run["model"],
