"""M5 — hypothesis testing: adjudicate M3's claims against corrected statistics.

M5 mostly does not compute. It routes a hypothesis's ``test_design`` to the M2
...
class HypothesisTestInput(WireModel):
    ...
    split: Literal["explore", "confirm", "all"] = Field(
        default="explore",
        description="Which partition M5 actually ran on. 'explore' means the hypotheses were "
...
class HypothesisTestOutput(StageEnvelope):
    ...
    def verified(self) -> list[str]:
        """Hypothesis ids M4 may operate on, strongest first.
