"""Langfuse-backed run reader for the EvalVitals HTML report.

Langfuse's observations API is row-oriented.  EvalVitals writes each durable
...
class LangfuseRunSource:
    """Read EvalVitals event envelopes from Langfuse Observations API v2."""
    ...
    def __init__(self, client: Any | None = None) -> None:
        if client is None:
            try:
                ...
                raise ImportError(
                    "Langfuse report input needs `pip install evalvitals[observability]`."
                ) from exc
            client = Langfuse()
        self.client = client

    def events(self, trace_id: str) -> list[dict[str, Any]]:
        """Return the exact EvalVitals events for *trace_id*, ordered by sequence."""
    ...
    def materialize(self, trace_id: str, destination: str | Path) -> Path:
        ...
        if not events:
            raise LookupError(f"No EvalVitals events found for Langfuse trace {trace_id!r}")
        for event in events:
