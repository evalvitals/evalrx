    ...
    *,
    ...
    def pai_forward(
        ...
        **kwargs: Any,
        ...
        if not getattr(attention, "_evalvitals_pai_suspend", False) and int(image_start) < stop:
...
class PAICFGLogitsProcessor:
    ...
    def _without_attention_boost(self) -> Iterator[None]:
        previous = [
            getattr(layer.self_attn, "_evalvitals_pai_suspend", False) for layer in self.layers
        ]
        try:
            for layer in self.layers:
                layer.self_attn._evalvitals_pai_suspend = True
            yield
        finally:
            for layer, value in zip(self.layers, previous):
                layer.self_attn._evalvitals_pai_suspend = value
