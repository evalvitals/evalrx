...
open model::
    ...
    from evalvitals.models.backends.gemini_compat import gemini_runtime
...
Torch-free; ``google-genai`` is imported lazily (``pip install 'evalvitals[gemini]'``).
"""
...
from evalvitals.models.blackbox.gemini import (
    _audio_bytes,
    _png_bytes,
    _to_genai_contents,
    _to_genai_tools,
...
class _Runtime:
    ...
    def genai(self):
        try:
            ...
            raise ImportError(
                "gemini_runtime needs the google-genai package: pip install 'evalvitals[gemini]'"
            ) from exc
        with self._lock:
