"""Shared M1→M4→Fix runner for the benchmark-backed VLM examples.
...
def main(config: BenchmarkConfig) -> None:
    parser = argparse.ArgumentParser(description=f"EvalRX M1-M4+Fix on {config.name}")
    ...
    if not discovery.has_m4_groups:
        raise RuntimeError("M4 needs both PASS and FAIL cases; adjust the frozen sample")
    ...
    outcome = loop.run_fix(
        ...
        max_tier=args.fix_tier,
        auto_escalate=args.auto_escalate,
        # No M4-verified hypothesis still gets a fix attempt on the best
