...
Full M1→M5 diagnosis loop for `Qwen2-Audio-7B-Instruct` on MMAU test-mini
...
see `docker-compose.yml`) — it's the M2/M3/M4 judge and FixAgent's L1/L2
...
loop.run_fix                  FixAgent's tiered candidates (default: only
                               ...
                               CONFIRM split the loop's own M1-M4 discovery
...
the same coder that runs the explore step and the M5 experiment — the same
...
on the *old* `m5/qwen2_audio_tcd_mmau` script — kept here as the reference
point for "generic fallback candidates on an audio task", not reproducible
byte-for-byte with this rewrite (different M1/M5 split, different hypothesis
source):
...
```bash
evalrx report examples/m1_m5/mmau_qwen2_audio/outputs \
  --example-dir examples/m1_m5/mmau_qwen2_audio
evalrx serve examples/m1_m5/mmau_qwen2_audio/outputs
```
