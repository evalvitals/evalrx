#!/usr/bin/env python3
"""mmau_qwen2_audio — Qwen2-Audio-7B-Instruct x MMAU test-mini: full M1→M5 loop.

Was ``examples/m5/qwen2_audio_tcd_mmau`` — a FixAgent-only run against a
...
hand-supplied hypothesis and lets the loop discover its own:
    ...
    5. loop.run_fix               FixAgent's tiered candidates — including the
                                   ...
                                   held-out CONFIRM split the loop's own M1–M4
...
def _run_tcd_confirmation(args: argparse.Namespace) -> int:
    ...
    This is deliberately separate from M1-M4: once a candidate was selected in
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="skip M1-M4 and confirm the already pilot-selected TCD candidate "
