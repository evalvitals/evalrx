...
M3 → held-out M4 → M5 → fix — with **Qwen3.5-2B loaded together with its
...
the CLI default provider is `agy`. With no M4-verified hypothesis the run
still enters M5 and the fix stage on the best unverified leads
...
| M3 / held-out M4 | 3 leads (critic kept 1); all inconclusive on 24 confirm cases |
| M5 | best lead refuted (metric_a 0.46 vs metric_b 3.0) |
...
| diagnosis | 2 leads, none verified on CONFIRM; M5 best lead refuted |
...
Checks that the merge kept our path: with `verified=0` M5 still ran on the
