...
> 走 README 的 Mode 1（容器提交）进入 `VLDiagnoseLoop`（M1→M4）；分析与修复由
> loop 内的现有 analyzer / tier-(b) 探针生成器 / M5 SurgeryAgent 完成，
...
- split：按 **image** 60/40 切 explore/validate（同图三 probe 永不跨 split）；
  所有调参只许碰 explore，validate 留给 M4/预注册检验一次性使用。
...
loop 的行为由 protocol 锚定（M1 选 analyzer、M4 拒绝跑题假设）。本 example 用：
...
## 6. 机理探测与修复的规格（给 M5 / GPU 侧 agent 的预期，不进包）

这两段是验收"loop 是否找对了原因"的参照答案，实现载体是 loop 自己
（tier-(b) 探针、M5 SurgeryAgent）或本目录内的脚本（见 TODO.md）：
...
- **探测信号**（每 case 一次 forward，pos=-1，`lm_head(norm(h_i))` 全词表
  ...
  - ⚠️ 设计上**接受被拒**：若信号不成立，结论是"Qwen3-VL 残余幻觉非 DeCo 型"，
    同样是合格归因，不强行复现论文。
- **修复方向**（M5）：DeCo 单步重打分——二元任务只有首 token 重要，无需接管
...
└── deco_fix.py          DeCo 单步重打分（M5 的 verify_fn 载体）
```
