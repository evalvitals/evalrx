...
Pipeline:
    ...
    or after --max-cycles cycles
         │
    M5  SurgeryAgent         claude/agy writes + runs targeted fix script
...
auto-generated README.txt for the full index:
    run_log.jsonl          ← one JSON line per event: run_start (config +
                              ...
                              diagnosis (M3), surgery (M4), tool_codegen +
                              tool_registry (tool synthesis), experiment (M5),
    ...
    experiments/             ← M5 generated script(s), run stdout/stderr, the
...
def _resolve_judge(args):
    ...
    """
    from evalrx.eval_agent import AgyModel, ClaudeModel
    ...
    def _try_agy():
        ...
        print(f"\n  judge : antigravity CLI ({judge._binary})  "
              f"model={name or 'session default'}  [M1–M4, no API key]")
    ...
    def _try_claude():
        ...
        print(f"\n  judge : claude CLI ({judge._binary})  "
              f"model={name or 'session default'}{effort_tag}  [M1–M4, no API key]")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Which CLI powers the M1–M4 judge (and the codegen stages). "
    ...
    # The coder provider for codegen stages (M1/M2 tier-b, M5 writer) follows
    ...
    print("  run_log.jsonl   ← one JSON line per event (run_start/M1/M2/M3/M4 + tool_codegen + experiment)")
    ...
    print("  experiments/    ← M5 generated script, output, thinking, verdict")
    ...
    if surgery_agent is not None:
        ...
        if outcome.fixed and outcome.best is not None:
            ...
            # no-verified-hypothesis skip path (loop.py's `if not hypotheses`
            # branch) carries only "reason"/"next_step" instead.
            if "recommend_tier" in rec:
                tier = rec["recommend_tier"] or "no higher tier available"
                print(f"  RECOMMEND  : raise the intervention tier to {tier}")
