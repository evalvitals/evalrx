...
Sibling of examples/m1_m5/deco_pope, but the failure slice is the *miss* (a present
...
able to close detect -> analyse -> *validated* repair.
    ...
    4. VLDiagnoseLoop M1->M4          (loop selects its own analyzers)
    5. run_m5 + run_fix               (loop proposes + validates its own fix)
...
# ---------------------------------------------------------------------------
# 3. Loop wiring (mirrors examples/m1_m5/deco_pope/run.py)
# ---------------------------------------------------------------------------
