...
The VLM IS loaded here: the fix module (and M5 surgery) call it to validate
candidate repairs against the unmodified baseline (paired McNemar / e-BH guard).
M4 confirmation itself reuses the persisted M2 stats — no re-analysis needed.
"""
...
def main() -> None:
    ...
    ap.add_argument("--backend", default="claude", choices=["claude", "codex", "agy"],
                    help="coder backend for M5/fix codegen (default claude)")
    ...
    # ── PHASE 2a: confirm the reloaded hypotheses with M4 ───────────────────
    ...
    # ── PHASE 2b: M5 surgery + tiered fix on the confirmed hypotheses ───────
    if not args.skip_m5:
