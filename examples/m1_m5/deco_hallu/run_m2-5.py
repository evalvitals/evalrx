"""deco_hallu — STAGE 2 of a split run: M2-M4 + M5 + fix, on a FROZEN M1 result.

Loads outputs/m1_state.pkl (produced by run_m1.py) and runs the rest of the
pipeline — M2 stats analysis, M3 hypotheses, M4 hypothesis testing, then M5
...
orchestration (and the M2-M4 stages) run exactly as in the single-shot run.py.
...
The model is still loaded here (M2 codegen, M5 surgery and the fix module call
...
def main() -> None:
    ...
    ap.add_argument("--backend", default="claude", choices=["claude", "codex", "agy"],
                    help="coder backend for M2/M5/fix codegen (default claude)")
    ...
    ap.add_argument("--explore-report", default="",
                    help="path to Step 1 fused_report.json — its charts/observations "
                         "(UNCONFIRMED) are shown to M3 only, never to M2/M4/fix")
    ...
    # enter M2/M3/M4 via the standard findings["per_case"] contract.
    signal_recipes = []
    if args.recipes:
    ...
    # NOT enter the M2 confirmatory family, M4 testing, or the fix gate.
    explore_report = None
