...
**Stage status:** designed as a full M1→M4 loop (diagnose the failure, then
repair it), but the runners here (`run_autofix.py` / `run_hf_autofix.py`)
only implement M4 — `FixAgent.propose_and_validate` against a hand-supplied
hypothesis (each paper's own `failure_axis`, or a real but diagnosis-split-only
self-diagnosis call — see `diagnose_hf()`). Real M1–M3 discovery (mine cases,
run analyzers, have the model itself propose the mechanism via
`VLDiagnoseLoop`) was never built for this corpus. Filed under `m4/` because
that's what actually runs today, not because M4-only was the design goal.
...
`examples/m1_m4/mllms_small_object/` with an `hf_local` model and
