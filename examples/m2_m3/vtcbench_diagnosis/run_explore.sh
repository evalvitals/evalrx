...
# every contributor's evalvitals checkout uses a .venv named exactly that,
...
TEMPLATE="$(python3 -c 'from evalvitals.analysis.trajectory_records import AGENT_QUESTION_TEMPLATE as t; print(t)')"
...
exec python3 -m evalvitals.cli explore "$RECORDS" \
  -q "$QUESTION" \
  --outcome-col label \
  --backend "$BACKEND" \
