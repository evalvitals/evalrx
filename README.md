...
hallucinations from correct rejections at **AUC 0.82** — then, unprompted, it
argued that its *second* strongest signal was an artifact of how attention was
extracted, not a real effect. It also marked its own best number as optimistic,
because the threshold had been chosen on the rows it was scored on.

**Most eval tools would have shipped you that second finding.**

[**Read the full run →**](examples/m2_statistics/deco_hallu_explore/reference_output/)
— 606 cases, 23 charts, 28 tables, every claim carrying an explicit
`do_not_infer`. Committed to this repo, unmodified. No install required.

<img src="examples/m2_statistics/deco_hallu_explore/reference_output/figures/attention_geometry_fail_vs_pass.png" alt="Attention geometry of hallucinations vs correct rejections across seven per-case signals, each panel labeled with its AUC and 95% confidence interval" width="100%">

<sub>Generated by the run, not by hand. Note the contrast is drawn only over
...
starts there: it explores raw eval logs, finds recurring patterns, proposes
falsifiable explanations, tests them on held-out cases, and validates candidate
interventions against the unchanged baseline. A run may end **inconclusive** —
...
| Run | What it shows |
|---|---|
| [**Attention & hallucination**](examples/m2_statistics/deco_hallu_explore/reference_output/) | 606 real VLM cases. Finds AUC 0.82, then attacks its own result. 1 of 4 candidate signals survives adjudication. |
| [**The confound catch**](examples/m2_statistics/synthetic_yield_explore/reference_output/) | Catalyst looks significant (ANOVA p = 0.080) until the run notices the groups differ by 21° in temperature. **0 of 4 signals confirmed** — the correct answer. |
...
adjudication. [Read the committed bundle →](examples/m2_statistics/synthetic_yield_explore/reference_output/)
...
|---|---|
| 📦 [`synthetic_yield_explore`](examples/m2_statistics/synthetic_yield_explore/reference_output/) | Standalone Explore on structured tabular outcomes — and a confound caught unprompted. |
| 📦 [`deco_hallu_explore`](examples/m2_statistics/deco_hallu_explore/reference_output/) | Explore → held-out hypothesis tests → tiered repair, on 606 real VLM cases. |
