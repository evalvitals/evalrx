...
def test_explore_adapter_keeps_validation_and_repair_distinct(tmp_path):
    ...
    assert view.stage(StageId.M5).state is StageState.NOT_STARTED


def test_loop_adapter_uses_action_order_and_maps_fix_event_to_m5(tmp_path):
    session = {
        "kind": "loop",
        "root": str(tmp_path),
        "story": {
            ...
            "surgeries": [{"event": "surgery", "module": "m4"}],
            "fixes": [{"event": "fix"}],
        },
    }
    ...
    assert [stage.id for stage in view.stages] == [
        StageId.M1, StageId.M2, StageId.M3, StageId.M4, StageId.M5,
    ]
    assert view.stage(StageId.M4).state is StageState.SUCCEEDED
    assert view.stage(StageId.M5).state is StageState.SUCCEEDED
    assert view.stage(StageId.M5).count == 1
