...
Extends :class:`~evalrx.analysis.analysis_module.AnalysisModule` with:
...
- **Richer output** (:class:`StatsAnalysisReport`): adds ``conclusion``,
  ...
  :class:`~evalrx.analysis.analysis_module.AnalysisReport` fields so M3
...
from evalrx.analysis.prompts.stats_agent import _ANALYSIS_PROMPT, _TOOL_SELECT_PROMPT
from evalrx.analysis.stats_tool_agent import StatsToolAgent as LegacyStatsToolAgent
...
class StatsAnalysisReport(AnalysisReport):
    ...
    Inherits all :class:`~evalrx.analysis.analysis_module.AnalysisReport`
...
class StatsAnalysisAgent:
    ...
    Args:
        judge:       Optional LLM for the LLM-guided analysis path.
                     Any :class:`~evalrx.core.model.Model` with
    ...
    def analyze(
        ...
        Returns:
            ...
            :class:`~evalrx.analysis.analysis_module.AnalysisReport`.
        """
    ...
    def _get_generator(self) -> "StatsToolGenerator | None":
        ...
        if self._judge is None and self._codegen_config is None:
            return None
        from evalrx.analysis.stats_tool_generator import StatsToolGenerator
...
def _multiplicity_note(result: StatsToolResult) -> str:
    ...
    ``correction_method`` is set by :func:`~evalrx.stats.multiplicity.correct_results`
