...
When no catalog tool in :mod:`~evalrx.analysis.stats_tools` fits the
data, this generator asks an LLM / CLI coding agent to *write* a small statistics
script, runs it inside an :class:`~evalrx.agent_runtime.sandbox.ExperimentSandbox`
...
- The generated script reads ``m2_stats_input.json`` (the serialised
  ...
  evalrx.stats`` to compose existing primitives or write novel statistics
  with numpy.
...
from evalrx.analysis.prompts.stats_tool_generator import (
    _GENERATE_PROMPT,
    _INPUT_FILENAME,
    _RESULT_MARKER,
)
from evalrx.analysis.stats_tools import StatsInput, StatsToolResult, describe_data
...
class StatsToolGenerator:
    ...
    Args:
        ...
        cli_config:  :class:`~evalrx.agent_runtime.cli_types.CliAgentConfig`
