...
pass/fail diagnosis logs — EvalRX runs it locally in a sandbox, and the
...
heuristic in :func:`evalrx.analysis.profile.profile_records` recognizes
...
from evalrx.agent_runtime.sandbox import ExperimentSandbox, SandboxResult
from evalrx.analysis.plain_language import jargon_violation
from evalrx.analysis.profile import describe_outcome, profile_records
from evalrx.analysis.prompts.explorer import (
...
class CandidateSignal:
    ...
    (:class:`~evalrx.analysis.stats_agent.StatsAnalysisAgent`, or the
    ...
    (:func:`evalrx.analysis.adjudicate.adjudicate_report`) recomputes the
...
class ExploratoryAnalysisReport:
    ...
    ``evalrx.analysis.hypothesis_agent.HypothesisAgent``), not by this
    ...
    # filled by evalrx.analysis.adjudicate; empty until adjudicated.
...
class ExploratoryAnalysisAgent:
    ...
    def __init__(
        ...
        # ones (eval-chart-style, nature-figure, evalrx-report-ui) so agent
    ...
    def explore_path(
        ...
        *,
        ...
        backend (``cli_config``, the ``evalrx explore`` default), the raw
    ...
    def _copy_raw_input(self, path: str | Path, *, max_files: int, include_tool_calls: bool) -> str:
        ...
        if root.is_file():
            ...
            from evalrx.analysis.workbench import SUPPORTED_SUFFIXES
...
def scan_folder(
    ...
    *,
    ...
    This is deliberately independent of :class:`~evalrx.analysis.profile.
...
def _parse_result_json(stdout: str) -> tuple[dict[str, Any], str]:
    ...
    :mod:`evalrx.analysis.result_marker` for the failure that motivated it.
    """
