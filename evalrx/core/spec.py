...
``compose(spec, backend)`` (see :mod:`evalrx.models.compose`) combines the
two into a :class:`~evalrx.core.model.Model`.
...
class AudioSpec:
    ...
    map that mirrors :class:`~evalrx.core.tokentype.TokenTypeMap` is Stage-2.
    """
