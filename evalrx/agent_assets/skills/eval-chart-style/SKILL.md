...
description: >
  ...
  figures. Use whenever you plot eval results inside an EvalRX analysis
...
`evalrx.analysis.eval_viz_theme` (plotly). If plotly is available you may
`from evalrx.analysis import eval_viz_theme as viz; viz.apply()` and use its
...
If `evalrx` isn't importable from the sandbox, inline the same rcParams
...
## Scope note for EvalRX sandboxes
