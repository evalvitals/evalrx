"""Console logging convenience for evalrx' existing per-module loggers.
...
application's own logging setup the moment ``import evalrx`` runs. The
...
Logging for a Library" section) is instead:
  ...
  * attach a :class:`logging.NullHandler` to the package's top-level logger
    (done in :mod:`evalrx` on import) so the library is silent — no
...
Usage::
    ...
    import evalrx
    evalrx.enable_console_logging()          # INFO and above, readable
    model = evalrx.load("qwen2.5-7b-instruct")
    ...                                            # now narrated to stdout

    evalrx.enable_console_logging(level=logging.DEBUG)  # noisier
    evalrx.disable_console_logging()                     # back to quiet
"""
...
# Every module's `logging.getLogger(__name__)` (e.g. "evalrx.eval_agent.loop")
# is a child of this in the dot-hierarchy, so one handler here reaches all of
# them via normal propagation -- except RunLogger's own "evalrx.run.<dir>"
...
_MARKER_ATTR = "_evalrx_console_handler"
...
def enable_console_logging(
    ...
    """Attach a readable stdout handler to evalrx' logger tree.
...
def disable_console_logging() -> None:
    ...
    themselves directly to ``logging.getLogger("evalrx")`` or below.
    """
    top_logger = logging.getLogger(TOP_LEVEL_LOGGER_NAME)
