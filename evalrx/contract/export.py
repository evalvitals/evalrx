"""Generate JSON Schema (and optionally TypeScript) from the Python contract.
    ...
    python -m evalrx.contract.export --out docs/contract
...
:mod:`evalrx.contract.typescript` for why not npx.
"""
...
from evalrx.contract import SCHEMA_VERSION, STAGE_IO
...
def export(out_dir: Path, *, typescript: bool = True, frontend: bool = True) -> list[Path]:
    ...
    for stage, contract in STAGE_IO.items():
        for role, model in (("Input", contract.input), ("Output", contract.output)):
            ...
            schema["$id"] = f"https://evalrx.dev/contract/{stage}.{model.__name__}.json"
    ...
    if typescript:
        ...
        from evalrx.contract.common import MEDIA_SLOTS, Modality
        from evalrx.contract.typescript import render
...
def _layout_doc() -> str:
    ...
    """
    from evalrx.contract.emit import CONTRACT_DIR, STAGE_WIRE
    ...
    if args.check:
        ...
        if stale:
            raise SystemExit(
                ...
                + f"\nregenerate with: python -m evalrx.contract.export --out {args.out}"
            )
