...
def _resolve_image(image: "Any") -> "Any | None":
    ...
    in :mod:`evalrx.core.case` — datasets like ``TextVQASizeDataset``
...
def attention_heatmap(
    ...
    ``model_attend()`` bridge (:mod:`evalrx.eval_agent.stages.fix_pipeline`):
...
class RelativeAttentionResult(Result):
    ...
    def plot(self, ax=None, figsize=(6, 6), cmap="hot"):
        ...
        try:
            ...
            raise ImportError(
                "Visualisation requires matplotlib: pip install evalrx[viz]"
            ) from None
...
class RelativeAttentionAnalyzer(Analyzer):
    ...
    Example::
        ...
        from PIL import Image
        from evalrx.analyzers.attention import RelativeAttentionAnalyzer
        from evalrx.core.case import Inputs
